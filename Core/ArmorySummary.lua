...
local function DisplayQuests(tooltip, characterInfo)
	...
	local numEntries;
	local questTitleText, level, questTag, isHeader, isCollapsed, isComplete, frequency, questID, label;
	local color;
 	...
 	if ( characterInfo.NumQuests > 0 ) then
        ...
        if ( not questHeaderState.Quests ) then
			...
			for i = 1, numEntries do
                questTitleText, level, questTag, isHeader, isCollapsed, isComplete, frequency, questID = Armory:GetQuestLogTitle(i);
				index, column = tooltip:AddLine("");
				...
				myColumn = column; index, column = tooltip:SetCell(index, myColumn, isHeader and "" or color..(questTag or "")..FONT_COLOR_CODE_CLOSE); 
			end
		end
	end
    ...
    for _, realmInfo in ipairs(summary) do
        ...
        for _, characterInfo in ipairs(realmInfo.Characters) do
            ...
            if ( not collapsed[realm] ) then
                ...
                if ( self:GetConfigSummaryQuest() and self:HasQuestLog() ) then
                    myColumn = column; index, column = self.summary:SetCell(index, myColumn, characterInfo.NumQuests or "", nil, "CENTER");
