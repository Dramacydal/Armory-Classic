...
local function SetInventoryValue(dbEntry, itemContainer, index, texture, count, quality, link, expires, equipable, slotId, ignorable, timestamp)
    dbEntry:SetValue(3, container, itemContainer, index, texture, count, quality, link, expires, equipable, slotId, ignorable, timestamp);
end
    ...
    if ( link and id >= BANK_CONTAINER and id <= NUM_BAG_SLOTS + NUM_BANKBAGSLOTS and IsEquippableItem(link) ) then
        ...
        if ( equipLoc == "INVTYPE_HAND" or equipLoc == "INVTYPE_WAIST" or equipLoc == "INVTYPE_CLOAK" ) then
            ...
            if ( id == BANK_CONTAINER ) then
                tooltip:SetInventoryItem("player", BANK_CONTAINER_INVENTORY_OFFSET + slotId);
            else
                tooltip:SetBagItem(id, slotId);
            end
            Armory:ReleaseTooltip(tooltip);
        end
    end

    SetInventoryValue(dbEntry, itemContainer, index, texture, count, quality, link, expires, equipable, slotId, ignorable);
end
    ...
    if ( not self:IsLocked(semaphore) ) then
        ...
        updater:Start(
            function(updater)
                ...
                local texture, count, quality, link, expires, equipable, slotId, ignorable, timestamp;
                for i = 1, #ArmoryInventoryContainers do
                    id = ArmoryInventoryContainers[i];
                    if ( id > ARMORY_AUCTIONS_CONTAINER ) then
                        ...
                        if ( not self:IsLocked(itemContainer) ) then
                            ...
                            if ( numSlots ) then
                                for index = 1, numSlots do
                                    texture, count, quality, link, expires, equipable, slotId, ignorable, timestamp = GetInventoryValue(dbEntry, itemContainer, index);
                                    if ( not equipable and link ) then
                                        equipable = self:CanEquip(link);
                                        SetInventoryValue(dbEntry, itemContainer, index, texture, count, quality, link, expires, equipable, slotId, ignorable, timestamp);
                                    end
                                end
                            end
...
function Armory:GetContainerItemLink(id, index)
    local _, _, _, link = self:GetInventoryContainerValue(id, index);
    return link;
end
...
function Armory:ScanInventory(link, reagentScan)
    ...
    return count, bagCount, bankCount, mailCount, auctionCount, perSlotCount;
end
...
function Armory:FindInventory(...)
                    ...
                    for index = 1, numSlots do
                        ...
                        if ( itemId and self:FindTextParts(text, ...) ) then
                            ...
                            if ( items[itemId] ) then
                                ...
                                items[itemId] = {link=link, count=itemCount, bags=0, bank=0, mail=0, alts=0, equipped=0};
                            end
    ...
    for _, info in pairs(items) do
        text = self:GetCountDetails(info.bags, info.bank, info.mail, nil, info.alts, info.equipped);
