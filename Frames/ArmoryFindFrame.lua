...
function ArmoryFindFrame_Add(who, where, what, link, tooltipLines, count)
    table.insert(ARMORY_FIND_RESULTS, {who=who, where=where, what=what, link=link, tooltipLines=tooltipLines, count=count});
end
...
function ArmoryFindFrameButton_OnEnter(self)
    ...
    if ( index and index <= #ARMORY_FIND_RESULTS ) then
        ...
        if ( link ) then
            ...
            Armory:SetHyperlink(GameTooltip, link);
        else
            local tooltipLines = ARMORY_FIND_RESULTS[index].tooltipLines;
